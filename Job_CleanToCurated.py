...
year_param = args["year"]        # renombradas para no sobrescribir funciones
month_param = str(int(args["month"]))

dyf_clean = glueContext.create_dynamic_frame.from_catalog(
    database="datalake-catalog-taxis",
    table_name="cleaned",
    push_down_predicate=f"(year == '{year_param}' and month == '{month_param}')"
)
df = dyf_clean.toDF()
